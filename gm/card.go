...
func addCard(values url.Values) []byte {
	...
	count, _ := strconv.ParseUint(values.Get("count"), 10, 32)
	if count == 0 {
		count = 1
	}
	for _, g := range game.Cache {
		game.Post(func() {
			if playerId < len(g.Players) && g.Players[playerId].IsAlive() {
				...
				g.Players[playerId].AddCards(c)
				logger.Info("由于GM命令，", g.Players[playerId], "摸了", c, "，现在有", len(g.Players[playerId].GetCards()), "张手牌")
				for _, player := range g.GetPlayers() {
					if player.Location() == playerId {
						player.NotifyAddHandCard(playerId, 0, c)
					} else {
						player.NotifyAddHandCard(playerId, 1)
					}
				}
			}
		})
